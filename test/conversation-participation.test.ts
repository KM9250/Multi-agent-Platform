...
import { partitionTurnDecisions, resolveTurnDecisions } from '../utils/turnDecisions.ts';
...
test('structured snapshots force only enabled target members while non-targets decide', async () => {
  ...
  const calls: string[] = [];
  const evaluateDecision = async (a: Agent) => { calls.push(a.id); return { outcome: 'IGNORE', source: 'llm_decision', latencyMs: 1 } as ResponseDecision; };
...
test('fallback requires text for questions and requests but permits acknowledgement stamps', () => {
  ...
  for (const content of ['A案とB案ならどちらがいいですか？', 'このコードをレビューしてください']) assert.equal(applyInitialUserTurnFallback([stamp(a), ignore(b)], [user(content)], 0).filter(x => x.decision.outcome === 'RESPOND').length, 1);
  for (const content of ['了解です。', 'ではA案で進めます。']) assert.equal(applyInitialUserTurnFallback([stamp(a), ignore(b)], [user(content)], 0).filter(x => x.decision.outcome === 'RESPOND').length, 0);
...
test('participation prompt excludes persona instructions and migrations normalize legacy state', () => {
  ...
  const prompt = buildParticipationDecisionPrompt(normalized, { recentMessageCount: 3, lastSpokenDistance: 2 });
