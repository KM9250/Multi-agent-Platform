...
test('snapshot validation rejects broken cross-object invariants', () => {
  ...
  assert.throws(() => validateCoordinationSnapshot({ ...base, sessions: { task: { ...base.sessions.task, initiator: 'outsider' } } }), /initiator/);
  assert.throws(() => validateCoordinationSnapshot({ ...base, sessions: { task: { ...base.sessions.task, participants: ['worker', 'reviewer'] } } }), /include the supervisor/);
